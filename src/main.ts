...
Sentry.init({
  ...
  release: `goldtrader@${__APP_VERSION__}`,
  environment: location.hostname === 'goldtradermonitor.vercel.app' ? 'production'
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('goldtrader-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('goldtrader-beta-mode', 'true');
    else localStorage.removeItem('goldtrader-beta-mode');
    location.reload();
  },
});
